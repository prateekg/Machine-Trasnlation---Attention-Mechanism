...
with open("D:/Text Data/Machine_Translation/deu.txt","r+", encoding="utf8") as file:
...
decoder = BahdanauDecoder(hidden_size,len(de_words),attn)
...
#    tk1 = tqdm_notebook(enumerate(en_inputs), total=len(en_inputs), leave=False)
    for i, sentence in tqdm(enumerate(en_inputs), total=len(en_inputs)):
        loss = 0.
